...
var baseutil = {
	...
	parseDate : function(str){
		try{
			var stime = str.replace(/-/g,':').replace(' ',':');
			stime = stime.split(':');
			var smonth = parseInt(stime[1]);
			return new Date(stime[0],smonth,stime[2],stime[3],stime[4],0);
		}catch(e){
			alert("日期转换发生异常:" + e);
			return null;
		}
...
function tm_selectAllBranch(cname){
	$("input[name='"+cname+"']").prop("checked", true);
}

function tm_selectNoBranch(cname){
	$("input[name='"+cname+"']").prop("checked", false);
}

function tm_unSelectAllBranch(cname){
...
function tmCheckBrowserSupport(){
	try{
		var tomjson = {"name":"tomexam","version":"3.0"};
		var jsonStr = JSON.stringify(tomjson);
		window.localStorage.setItem("softinfo", jsonStr);
		var lsdata = window.localStorage.getItem("softinfo");
		var lsjson = JSON.parse(lsdata);
		lsjson["auth"] = "all";
		return true;
	}catch(e){
		return false;
